package rwc_ContigFilter::rwc_ContigFilterClient;
...
rwc_ContigFilter::rwc_ContigFilterClient

=head1 DESCRIPTION


A KBase module: rwc_ContigFilter
...
sub new
{
    ...
    my $self = {
	client => rwc_ContigFilter::rwc_ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a rwc_ContigFilter.FilterContigsParams
$return is a rwc_ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a rwc_ContigFilter.workspace_name
	contigset_id has a value which is a rwc_ContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a rwc_ContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "rwc_ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "rwc_ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for rwc_ContigFilter::rwc_ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "rwc_ContigFilter::rwc_ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package rwc_ContigFilter::rwc_ContigFilterClient::RpcClient;
