...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'rwc_ContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from rwc_ContigFilter.rwc_ContigFilterImpl import rwc_ContigFilter
impl_rwc_ContigFilter = rwc_ContigFilter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['rwc_ContigFilter.filter_contigs_async'] = ['rwc_ContigFilter', 'filter_contigs']
async_check_methods['rwc_ContigFilter.filter_contigs_check'] = ['rwc_ContigFilter', 'filter_contigs']
sync_methods['rwc_ContigFilter.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'rwc_ContigFilter'
        ...
        self.rpc_service.add(impl_rwc_ContigFilter.filter_contigs,
                             name='rwc_ContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['rwc_ContigFilter.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "rwc_ContigFilter but no authentication header was passed"
                            raise err
